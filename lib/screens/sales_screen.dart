...
class _SalesViewState extends State<SalesView> {
  ...
  double _itemProfit(InventoryItem item) {
    return item.price * item.quantity;
  }

  double _maxProfit(List<InventoryItem> items) {
    if (items.isEmpty) return 0;
    return items.map(_itemProfit).reduce((a, b) => a > b ? a : b);
  }

  bool _isLowProfit(InventoryItem item, double maxProfit) {
    return _itemProfit(item) <= maxProfit * 0.2;
  }

  String _calculateNetProfit(List<InventoryItem> items) {
    final total = items.fold<double>(0, (sum, item) => sum + _itemProfit(item));
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        // Shortcut container placed next to profit
