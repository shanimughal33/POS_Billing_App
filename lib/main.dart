      ...
      child: MaterialApp(
        ...
        onGenerateRoute: (settings) {
          ...
          if (settings.name == '/sales') {
            page = BlocBuilder<InventoryCubit, InventoryState>(
              builder: (context, state) {
                // Use cached items if available
                final inventoryCubit = context.read<InventoryCubit>();
                if (inventoryCubit.items.isNotEmpty) {
                  return BlocProvider<SalesCubit>(
                    create: (_) => SalesCubit(inventoryCubit.items),
                    child: const CustomerBillScreen(),
                  );
                }

                // If no cached items, show loading
                ...
                // If loaded, use the state items
                if (state is InventoryLoaded) {
                  return BlocProvider<SalesCubit>(
                    create: (_) => SalesCubit(state.items),
                    child: const CustomerBillScreen(),
                  );
                }

                // For any other state, trigger a refresh and show loading
                inventoryCubit.refreshInventory();
                return const Scaffold(
                  body: Center(
                    child: CircularProgressIndicator(
                      valueColor: AlwaysStoppedAnimation<Color>(
...
class _CustomerBillScreenState extends State<CustomerBillScreen>
  ...
  static const Duration _shortcutEntryTimeout = Duration(milliseconds: 1000);
  ...
  void _handleCreateBill() async {
    ...
    try {
      ...
      // Mark current items as completed before navigating
      await context.read<SalesCubit>().completeBill();
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    SizeTransition(
                      sizeFactor:
                          _searchAnimation ?? const AlwaysStoppedAnimation(0.0),
                ...
                ElevatedButton.icon(
                  ...
                  icon: Icon(Icons.receipt_long, size: 18), // Reduced icon size
                  label: Text(
                    "Create Bill",
